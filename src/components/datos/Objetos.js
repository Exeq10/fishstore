export const Objetos = [
  {
    id: 1,
    nombre: "Reel Okuma",
    tipo: "Frontal",
    precio: 1245,
    picture: "/images/reel frontal1.jpg",
    stock: 40,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Repellendus pariatur quis distinctio sunt quod corrupti, earum atque hic aspernatur eveniet aliquid totam veniam debitis dolore ut repudiandae, fugit voluptates reprehenderit.",
    categoria: "reel",
  },

  {
    id: 2,
    nombre: "Reel Rapala",
    tipo: "Rotativo",
    precio: 1685,
    picture: "/images/reelrotativo1.jpg",
    stock: 30,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Repellendus pariatur quis distinctio sunt quod corrupti, earum atque hic aspernatur eveniet aliquid totam veniam debitis dolore ut repudiandae, fugit voluptates reprehenderit.",
    categoria: "reel",
  },

  {
    id: 3,
    nombre: "Caña de lance Okuma",
    tipo: " Carbono",
    precio: 6745,
    picture: "/images/caña2.jpg",
    stock: 14,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Repellendus pariatur quis distinctio sunt quod corrupti, earum atque hic aspernatur eveniet aliquid totam veniam debitis dolore ut repudiandae, fugit voluptates reprehenderit.",
    categoria: "caña",
  },
  {
    id: 4,
    nombre: "Tanza Sufix 30 mm",
    tipo: "Nylon",
    precio: 800,
    picture: "/images/tanzasufix.jpg",
    stock: 40,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Repellendus pariatur quis distinctio sunt quod corrupti, earum atque hic aspernatur eveniet aliquid totam veniam debitis dolore ut repudiandae, fugit voluptates reprehenderit.",
    categoria: "tanza",
  },
  {
    id: 5,
    nombre: "Tanza Vexter 60 mm",
    tipo: "Multifilamento",
    precio: 1205,
    picture: "/images/tanzavexter.jpg",
    stock: 12,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Repellendus pariatur quis distinctio sunt quod corrupti, earum atque hic aspernatur eveniet aliquid totam veniam debitis dolore ut repudiandae, fugit voluptates reprehenderit.",
    categoria: "tanza",
  },
  {
    id: 6,
    nombre: "Reel xtreme",
    tipo: "Frontal",
    precio: 6845,
    picture: "/images/reel rotativo3.webp",
    stock: 10,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Repellendus pariatur quis distinctio sunt quod corrupti, earum atque hic aspernatur eveniet aliquid totam veniam debitis dolore ut repudiandae, fugit voluptates reprehenderit.",
    categoria: "reel",
  },
  {
    id: 7,
    nombre: "Tanza Mustad ",
    tipo: "Nylon",
    precio: 1789,
    picture: "/images/mustad0.7.jpg",
    stock: 12,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Repellendus pariatur quis distinctio sunt quod corrupti, earum atque hic aspernatur eveniet aliquid totam veniam debitis dolore ut repudiandae, fugit voluptates reprehenderit.",
    categoria: "tanza",
  },
  {
    id: 8,
    nombre: "Caña Review",
    tipo: "Fibra",
    precio: 1200,
    picture: "/images/cañareview.jpg",
    stock: 18,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Repellendus pariatur quis distinctio sunt quod corrupti, earum atque hic aspernatur eveniet aliquid totam veniam debitis dolore ut repudiandae, fugit voluptates reprehenderit.",
    categoria: "caña",
  },
  {
    id: 9,
    nombre: "Caña Robalo",
    tipo: "Carbono",
    precio: 4745,
    picture: "/images/cañarobalo.jpg",
    stock: 15,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Repellendus pariatur quis distinctio sunt quod corrupti, earum atque hic aspernatur eveniet aliquid totam veniam debitis dolore ut repudiandae, fugit voluptates reprehenderit.",
    categoria: "caña",
  },
  {
    id: 10,
    nombre: "Anzuelo Mustad",
    tipo: "Paquete de 50",
    precio: 50,
    picture: "/images/anzueloMustad.jpg",
    stock: 100,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Repellendus pariatur quis distinctio sunt quod corrupti, earum atque hic aspernatur eveniet aliquid totam veniam debitis dolore ut repudiandae, fugit voluptates reprehenderit.",
    categoria: "accesorio",
  },
  {
    id: 11,
    nombre: "Anzuelo Robador",
    tipo: "Paquete de 30",
    precio: 60,
    picture: "/images/anzueloRobador.jpg",
    stock: 150,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Repellendus pariatur quis distinctio sunt quod corrupti, earum atque hic aspernatur eveniet aliquid totam veniam debitis dolore ut repudiandae, fugit voluptates reprehenderit.",
    categoria: "accesorio",
  },
  {
    id: 12,
    nombre: "Plomada Redonda",
    tipo: "Plomo 100gr",
    precio: 15,
    picture: "/images/plomadaredonda100gr.jpg",
    stock: 150,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Repellendus pariatur quis distinctio sunt quod corrupti, earum atque hic aspernatur eveniet aliquid totam veniam debitis dolore ut repudiandae, fugit voluptates reprehenderit.",
    categoria: "accesorio",
  },
  {
    id: 13,
    nombre: "Plomada Triangular",
    tipo: "Plomo 125gr",
    precio: 25,
    picture: "/images/plomadaRombo120gr.jpg",
    stock: 120,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Repellendus pariatur quis distinctio sunt quod corrupti, earum atque hic aspernatur eveniet aliquid totam veniam debitis dolore ut repudiandae, fugit voluptates reprehenderit.",
    categoria: "accesorio",
  },
  {
    id: 14,
    nombre: "Señuelo Raton",
    tipo: "Señuelo de rio",
    precio: 250,
    picture: "/images/señueloRaton.jpg",
    stock: 120,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Repellendus pariatur quis distinctio sunt quod corrupti, earum atque hic aspernatur eveniet aliquid totam veniam debitis dolore ut repudiandae, fugit voluptates reprehenderit.",
    categoria: "accesorio",
  },
  {
    id: 15,
    nombre: "Señuelo pez",
    tipo: "Señuelo de mar",
    precio: 280,
    picture: "/images/señuelo1.jpeg",
    stock: 120,
    description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Repellendus pariatur quis distinctio sunt quod corrupti, earum atque hic aspernatur eveniet aliquid totam veniam debitis dolore ut repudiandae, fugit voluptates reprehenderit.",
    categoria: "accesorio",
  },
];
